...
def extract_planes(
    ...
    for view_name, perm, should_scale_z in permutations:
        ...
        if should_scale_z and anisotropy > 1.0:
            ...
            mask_perm = transforms.resize_image(
                mask_perm,
                Ly=new_Ly,
                Lx=Lx,
                no_channels=True,  # <- treat first dim as z, not channels
        ...
        for i, z_idx in enumerate(slice_indices):
            ...
            if sharpen_radius:
                img_slice = transforms.smooth_sharpen_img(
                    img_slice, sharpen_radius=sharpen_radius
                )
...
def main():
    ...
    pairs = get_image_mask_pairs(
        args.dir, img_filter=args.img_filter if args.img_filter else None
    )
    ...
    for image_path, mask_path in pairs:
        ...
        try:
            ...
            img_3d = transforms.convert_image(
                img_3d, channel_axis=args.channel_axis, z_axis=args.z_axis, do_3D=True
            )
            mask_3d = transforms.convert_image(
                mask_3d, channel_axis=args.channel_axis, z_axis=args.z_axis, do_3D=True
            )
