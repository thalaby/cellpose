...
def load_info(sample):
    """Helper function to load JSON info from a sample tuple."""
    _, info = sample
...
class SA1BDataset:
    ...
    def __init__(self, dataset_dir,ids=None,annotation_dir='annotations',image_dir='images',min_object=0):
        ...
        self.samples = [load_sample(dataset_dir, id, annotation_dir, image_dir) for id in tqdm(ids, desc="Loading samples")]
        self.image_info=[]

        with Pool(processes=os.cpu_count()) as pool:
            self.image_info = list(tqdm(pool.imap(load_info, self.samples), total=len(self.samples), desc="Loading image information"))
    ...
    def __getitem__(self, index):
        ...
        return {"pixel_values": img, "labels": mask, "class_ids": class_ids}
